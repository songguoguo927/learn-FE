//线性（顺序）查找 最佳/最差/平均情况O(n)
//最低效的一种搜索算法：将每一个（意味着迭代整个数组）数据结构中的元素和我们要找的目标元素比较
...
//二分查找（折半查找）  最佳/最差/平均情况O(logn) 时间复杂度的算法随着数据规模的增⼤，它的优势就越明显。
//这个算法要求查找表的数据是线性结构存储，还要求查找表中的顺序是由小到大或由大到小排序
